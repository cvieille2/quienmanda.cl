...
email_rules = [
    ...
    ('montavord', 'info@montavord.cl'),
    ('bristol', 'reservas@motelbristol.cl'),
]
...
phone_rules = [
    ...
    ('yugos', '+56652293367'),
    ('los-gatitos-macul', '+56222212784'),
    ('melosas', '+56752441109'),
    ('liguria', '+56412933553'),
    ('giralda', '+56222811939'),
    ('montavord', '+56228394027'),
    ('deja-vu', '+56522226409'),
    ...
    ('bristol-valparaiso', '+56322291088'),
    ('cabanas-rauquen', '+5657222066'),
]

def match(slug, rules):
    for kw, val in rules:
        if kw in slug:
            return val
    return ''
...
def get_name(slug):
    ...
    if len(parts) >= 2:
        return parts[-1].replace('-', ' ').title(), parts[0].replace('-', ' ').title()
    return parts[0].replace('-', ' ').title(), ''

def email_body(name, city, clics, imp, ctr, pos, email):
...
for f in fichas:
    ...
    email = match(slug, email_rules)
    phone = match(slug, phone_rules)

    if email:
        lines.append(f"## {name} ({city})\n")
        lines.append(f"- URL: https://infomoteles.cl/{slug}/\n")
        lines.append(f"- Email: {email}\n")
        lines.append(f"- Datos SC: {imp} imp, {clics} clics, {ctr}% CTR, pos {pos}\n\n")
        lines.append("```\n" + email_body(name, city, clics, imp, ctr, pos, email) + "\n```\n\n---\n")
        email_batch.append((slug, email, name, city, clics, imp, ctr, pos))
    elif phone:
        lines.append(f"## {name} ({city})\n")
        lines.append(f"- URL: https://infomoteles.cl/{slug}/\n")
        lines.append(f"- WhatsApp/Tel: {phone}\n")
        lines.append(f"- Datos SC: {imp} imp, {clics} clics\n\n")
        lines.append("WhatsApp:\n```\n" + wa_body(name, city, imp) + "\n```\n\n---\n")
        wa_batch.append((slug, phone, name, city, imp))
    else:
        no_list.append((slug, name, city, imp))
        no_count += 1

lines.append(f"\n## Sin Contacto ({no_count})\n\n")
lines.append("Prioridad: buscar email/WhatsApp de estos moteles.\n\n")
for slug, name, city, imp in no_list:
    lines.append(f"- [{name} ({city})](https://infomoteles.cl/{slug}/) - {imp} impresiones\n")

lines.append(f"\n---\n## Resumen Final\n\n")
lines.append(f"- **Con email:** {len(email_batch)}\n")
lines.append(f"- **WhatsApp listo:** {len(wa_batch)}\n")
...
for slug, email, name, city, clics, imp, ctr, pos in email_batch:
    ...
    with open(f'entregables/emails_listos/{safe}.eml', 'w', encoding='utf-8') as f:
        f.write(email_body(name, city, clics, imp, ctr, pos, email))

wa_by_city = {}
for slug, phone, name, city, imp in wa_batch:
    wa_by_city.setdefault(city, []).append((phone, name, imp, slug))
...
with open('entregables/whatsapp_listos/README.md', 'w', encoding='utf-8') as f:
    ...
    for city in sorted(wa_by_city.keys()):
        f.write(f"## {city}\n\n")
        for phone, name, imp, slug in wa_by_city[city]:
            f.write(f"### {name} - {phone}\n")
            f.write(f"URL: https://infomoteles.cl/{slug}/\n\n")
            f.write(f"```\n{wa_body(name, city, imp)}\n```\n\n")

print("OUTREACH GENERADO EXITOSAMENTE")
print(f"  Emails: {len(email_batch)}")
print(f"  WhatsApp: {len(wa_batch)}")
print(f"  Sin contacto: {no_count}")
print(f"\nArchivos:")
print(f"  entregables/outreach_completo.md")
print(f"  entregables/emails_listos/ ({len(email_batch)} archivos)")
print(f"  entregables/whatsapp_listos/ ({len(wa_batch)} mensajes)")
