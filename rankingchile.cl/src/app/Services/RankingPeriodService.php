...
class RankingPeriodService
{
    ...
    public function activePeriod(): ?RankingPeriod
    {
        return RankingPeriod::query()
            ->where('status', RankingPeriodStatus::Active)
            ->latest('id')
            ->first();
    }
     ...
     */
    public function periodFor(CarbonImmutable $at): RankingPeriod
    {
     ...
     */
    public function closeExpiredAndSettle(?CarbonImmutable $now = null): ?RankingPeriod
    {
        $moment = $now ?? now()->timezone(self::TZ);
        ...
        $pending = RankingPeriod::query()
            ->where('status', RankingPeriodStatus::ClosedPendingSettlement)
            ->lockForUpdate()
            ->get();
    ...
    public function ensureActivePeriod(CarbonImmutable $at): void
    {
    ...
    private function rangeFor(CarbonImmutable $at): array
    {
